...
def createDataCSV (augdir = "./NYU_UW_type1/", gtdir = "./NYU_GT/", outfile="data_typeIII_NL.csv"):
    df = pd.DataFrame(columns=['AUGFILE', 'GTFILE'])
    for i in range (1, 1500):
        indexSTR = str(i)
        df = pd.concat([df, pd.DataFrame.from_records([{'AUGFILE': str(augdir+"uw ("+indexSTR+").bmp"), 'GTFILE' : str(gtdir+"gt ("+indexSTR+").bmp")}])])
        #df = df.append({'AUGFILE': str(augdir+indexSTR+"_UWImage_1_.bmp"), 'GTFILE' : str(gtdir+indexSTR+"_Image_.bmp")}, ignore_index=True)
        #df = pd.concat({'AUGFILE': str(augdir+"uw\("+indexSTR+"\).bmp"), 'GTFILE' : str(gtdir+"gt\("+indexSTR+"\).bmp")}, ignore_index=True)
        #df = df.append(
...
if __name__ == "__main__":
    createDataCSV(augdir="./NYU_UW_type1/", outfile="data_type1.csv")
